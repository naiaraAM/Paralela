...
void Multiplicar_Matrices_Inf(float *A, float *B, float *C, int dim, int row_start, int row_end);
...
int main (int argc, char ** argv)
{
    ...
    // Both init to sup so they have data to multiply
    Init_Mat_Inf (dim, A);
    Init_Mat_Sup (dim, B);
    ...
    {
        row_start[i] = i * rows_per_thread + 1;
        row_end[i] = (i + 1) * rows_per_thread + 1;
    ...
    {
        threads[i] = std::thread(Multiplicar_Matrices_Inf, A, B, C, dim, row_start[i], row_end[i]);
    }
    for (auto i = 0; i < num_threads; i++)
    {
        threads[i].join();
