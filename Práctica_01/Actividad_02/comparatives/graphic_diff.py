...
datos_threads_inf = [data[i:i+27] for i in range(27, len(data), 27)]
datos_omp = [data[i:i+27] for i in range(54, len(data), 27)]
datos_omp_inf = [data[i:i+27] for i in range(81, len(data), 27)]

# Crear un gráfico para cada conjunto de datos
plt.plot(x, datos_threads[0], label="Threads")
plt.plot(x, datos_threads_inf[0], label="Threads, Inf")
plt.plot(x, datos_omp[0], label="OpenMP")
plt.plot(x, datos_omp_inf[0], label="OpenMP, Inf")
